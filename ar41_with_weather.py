    ...
    for date in date_list:
        ...
        if not result_filename_pollen.exists():
            ...
            if response2.status_code == 200:
                ...
                with open(result_filename_pollen, "w", encoding="utf-8") as fichier_json:
                    json.dump(response_data, fichier_json)
...
def add_weather_data(
    ...
    for date in date_list:
        date = date.split("T")[0]
        pollen_data_filename = weather_data_folder / (date + "pollen.json")  # get data from weather_data_folder
        ...
        for index in sub_data.index.to_list():
            ...
            data.loc[index, "temperature"] = weather_date["hourly"]["temperature_2m"][sub_data_hour_index]
            data.loc[index, "precipitation"] = weather_date["hourly"]["precipitation"][sub_data_hour_index]
            data.loc[index, "windspeed_10m"] = weather_date["hourly"]["windspeed_10m"][sub_data_hour_index]
...
if __name__ == "__main__":
    counter = 0
    chuck_folder = Path("chucks3")
    chuck_folder.mkdir(exist_ok=True)
    with tqdm(postfix=f"Cluster {counter}") as progress_bar:
        for chuck in pd.read_csv("ar41_for_ulb.csv", delimiter=";", parse_dates=True, index_col="timestamps_UTC", chunksize=100000):
            ...
            counter += 1
            progress_bar.postfix = f"Cluster {counter+1}"
